package com.joephone.codebox.act.list.view;
 ...
 * on 14-4-30.
 */
public class DropBackListView extends ListView {
	...
	public DropBackListView(Context context) {
		super(context);
		this.mContext = context;
		initView();
	}

	public DropBackListView(Context context, AttributeSet attrs) {
		super(context, attrs);
		this.mContext = context;
		this.attrs = attrs;
		initView();
	...
	public DropBackListView(Context context, AttributeSet attrs, int defStyle) {
	...
	private void initView() {
		...
		TypedArray a = mContext.obtainStyledAttributes(attrs, R.styleable.DropBackListView);
		mImageId = a.getResourceId(R.styleable.DropBackListView_headimage, 0);
		a.recycle();
	...
	private void initHead() {
		...
		headerView = inflater.inflate(R.layout.list_drop_back_top_img, null);
		...
		LayoutParams relativeLayout = new LayoutParams((int) imgWidth, (int) imgHeight);
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (mode == MODE_DRAG) {
				...
				if (dy / 2 + imgHeight <= 1.5 * imgHeight) {
					...
					if (dy > 0) {
						scaleY = dy;
						LayoutParams relativeLayout = new LayoutParams((int) (scale * imgWidth), (int) (scale * imgHeight));
						imageView.setLayoutParams(relativeLayout);
